...
function applyLanguage(lang) {
  ...
  const setTxt = (id, str) => { const el = document.getElementById(id); if (el && str) el.textContent = str; };
  setTxt('popMoviesHeaderTitle', t.popularMovies);
  setTxt('popTvHeaderTitle', t.popularTv);
  setTxt('actionHeaderTitle', t.actionAdventure);
  setTxt('scifiHeaderTitle', t.scifiFantasy);
  setTxt('crimeHeaderTitle', t.crimeThrillers);
  setTxt('comedyHeaderTitle', t.comedy);
  setTxt('topRatedHeaderTitle', t.topRated);
  setTxt('contWatchHeaderTitle', t.continueWatching);
...
function isUpcomingTitle(item) {
  if (!item) return false;
  if (item.isUpcoming) return true;
  const year = parseInt(item.year || item.releaseInfo || '0', 10);
  if (year >= 2026) return true;
  const unreleased = [
    'project hail mary', 'avengers: doomsday', 'spider-man: brand new day', 
    'the batman: part ii', 'avatar: fire and ash', 'fantastic four: first steps',
    'superman', 'apex', 'the odyssey', 'tron: ares', 'blade', 'shrek 5', 'fast x: part 2', 'michael', 'zootopia 2'
  ];
  const name = (item.name || item.title || '').toLowerCase().trim();
  return unreleased.some(t => name.includes(t));
}

// Dynamic Initial Data from Real Cinemeta Catalog (100% Live API)
async function setupInitialSeedData() {
  try {
    const [mTopRes, sTopRes] = await Promise.allSettled([
    ...
    let movies = (mTopRes.status === 'fulfilled' && mTopRes.value?.metas) || [];
    let shows = (sTopRes.status === 'fulfilled' && sTopRes.value?.metas) || [];

    if (movies.length > 0) {
      registerItems(movies);
      movieIds = Array.from(new Set(movies.map(m => m.id)));
      activeMedia = mediaMap.get(movieIds[0]);
      currentHeroMedia = activeMedia;
    }
    ...
    if (loadMoreMoviesWrap && movieIds.length > 0) loadMoreMoviesWrap.style.display = 'flex';

    const loadMoreTvWrap = document.getElementById('loadMoreTvWrap');
    if (loadMoreTvWrap && tvIds.length > 0) loadMoreTvWrap.style.display = 'flex';

    // Deep background load of genres
...
function applyMovieFilters() {
  ...
  if (activeMovieYear && activeMovieYear !== 'all') {
    list = list.filter(m => String(m.year).includes(String(activeMovieYear)));
  }
...
window.navigateTo = function(tabName, mediaId = null) {
  ...
  if (tabName === 'home') {
    if (currentHeroMedia) {
      activeMedia = currentHeroMedia;
      setupSpotlightHero(currentHeroMedia);
    }
...
function renderAllViews() {
  ...
  const setRow = (id, list) => {
    const el = document.getElementById(id);
    if (!el) return;
    if (list && list.length > 0) {
      el.innerHTML = list.map(item => createCardHtmlById(item.id || item)).join('');
    }
  };

  // 1. Popular Movies
  setRow('popularMoviesRow', allMovies.slice(0, 30));

  // 2. Popular TV Shows
  setRow('popularShowsRow', allShows.slice(0, 30));

  // 3. Action & Adventure
  const actionList = allMovies.filter(m => /action|adventure/i.test((m.genres || []).join(' ') + ' ' + (m.genre || '')));
  setRow('actionMoviesRow', actionList.length > 0 ? actionList.slice(0, 30) : allMovies.slice(0, 20));

  // 4. Sci-Fi & Fantasy
  const scifiList = allMovies.filter(m => /sci-fi|fantasy|space|superhero/i.test((m.genres || []).join(' ') + ' ' + (m.genre || '')));
  setRow('scifiMoviesRow', scifiList.length > 0 ? scifiList.slice(0, 30) : allMovies.slice(5, 25));

  // 5. Crime & Thrillers
  const crimeList = allMovies.filter(m => /crime|thriller|mystery|suspense/i.test((m.genres || []).join(' ') + ' ' + (m.genre || '')));
  setRow('crimeMoviesRow', crimeList.length > 0 ? crimeList.slice(0, 30) : allMovies.slice(10, 30));

  // 6. Comedy & Entertainment
  const comedyList = allMovies.filter(m => /comedy|animation|family/i.test((m.genres || []).join(' ') + ' ' + (m.genre || '')));
  setRow('comedyMoviesRow', comedyList.length > 0 ? comedyList.slice(0, 30) : allMovies.slice(15, 35));

  // 7. Top Rated Cinema
  const topRatedList = [...allMovies].sort((a, b) => parseFloat(b.imdbRating || 0) - parseFloat(a.imdbRating || 0));
  setRow('topRatedRow', topRatedList.slice(0, 30));
...
// Multi-Source Addon & Server Stream Generator (Server 4: VidSrc Mirror 4K ONLY)
function generateServerUrls(imdbId, isSeries, season = 1, episode = 1) {
  if (isSeries) {
    return [
      {
        name: 'Server 4 · VidSrc Mirror 4K',
        sub: 'Alternative HD/4K Node',
        url: `https://vidsrc.pm/embed/tv?imdb=${imdbId}&season=${season}&episode=${episode}`
      }
    ];
  }
  return [
    {
      name: 'Server 4 · VidSrc Mirror 4K',
      sub: 'Alternative HD/4K Node',
      url: `https://vidsrc.pm/embed/movie?imdb=${imdbId}`
    }
  ];
}
...
async function fetchAndDisplayStreams(item, season = 1, episode = 1) {
  ...
  // Directly start playback on Server 4 without showing release picker modal
  addToHistory(item, season, episode);
  launchVenticPlayer(item.name, servers[0].url);

  /* =========================================================================
   * RELEASES PICKER LOGIC (Commented out - preserved for future use)
   * =========================================================================
   * const epInfo = isSeries ? ` (S${season}:E${episode})` : '';
   * if (elements.streamPickerTitle) elements.streamPickerTitle.textContent = `${item.name}${epInfo}`;
   * if (elements.streamPickerModal) elements.streamPickerModal.style.display = 'flex';
   * if (elements.streamList) {
   *   elements.streamList.innerHTML = `<div class="loading-pulse" style="padding: 1.5rem;"><i class="fa-solid fa-spinner fa-spin"></i> Searching across YTS, 1337x, TorrentGalaxy...</div>`;
   * }
   * const streamTarget = isSeries ? `${imdbId}:${season}:${episode}` : imdbId;
   * const type = item.type || 'movie';
   * const providerEndpoints = [
   *   `https://torrentio.strem.fun/stream/${type}/${streamTarget}.json`,
   *   `https://torrentio.strem.fun/sort=qualitysize|qualityfilter=scr,cam/stream/${type}/${streamTarget}.json`
   * ];
   * ========================================================================= */
}
...
function onStreamSelected(idx) {
  ...
  if (!playUrl || playUrl.startsWith('magnet:')) {
    ...
    playUrl = servers[0].url;
  }

  launchVenticPlayer(activeMedia.name, playUrl);
}

// ==========================================================================
// CINEMAIQ MULTI-SERVER STREAM ENGINE
// ==========================================================================
...
function launchVenticPlayer(title, streamUrl) {
  ...
  if (elements.hudMediaTitle) elements.hudMediaTitle.textContent = title || activeMedia?.name || 'Now Playing';
  ...
  const iframe = document.getElementById('cinemaStreamIframe');
  ...
  try {
    if (!window.history.state || !window.history.state.cinemaPlayerOpen) {
      window.history.pushState({ cinemaPlayerOpen: true }, '', window.location.href);
    }
  } catch (err) {}
}

function exitVenticPlayer(popHistory = true) {
...
// Mobile Hardware & Browser Swipe-to-Back Handler
window.addEventListener('popstate', (e) => {
  if (isPlayerActive) {
    exitVenticPlayer(false);
  }
});
...
function initCustomPlayer() {
  ...
  const mobileFloatingBack = document.getElementById('mobileFloatingBackBtn');
  ...
  if (mobileFloatingBack) {
    mobileFloatingBack.addEventListener('click', (e) => {
      e.stopPropagation();
      exitVenticPlayer();
    });
  }
...
function setupGlobalEventDelegation() {
  ...
  if (elements.heroPlayBtn) {
    elements.heroPlayBtn.addEventListener('click', () => {
      const target = currentHeroMedia || activeMedia;
      if (target) {
        activeMedia = target;
        if (target.type === 'series' || target.isAnime) {
          openDedicatedDetailPage(target);
        } else {
          addToHistory(target, 1, 1);
          fetchAndDisplayStreams(target, 1, 1);
        }
      }
    });
  }
  ...
  if (elements.heroHeartBtn) {
    elements.heroHeartBtn.addEventListener('click', () => {
      const target = currentHeroMedia || activeMedia;
      if (target) toggleFavourite(target);
    });
  }
